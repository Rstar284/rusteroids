...
impl Asteroid {
    ...
    fn shape<R>(rng: &mut R, radius: f64) -> Vec<Point>
    where
        R: Rng + ?Sized,
    {
    ...
    pub fn grid<R>(rng: &mut R, cols: u32, rows: u32) -> Vec<Asteroid>
    where
        R: Rng + ?Sized,
    {
    ...
    pub fn field<R>(rng: &mut R, bounds: &Size, count: u32, clearing: f64) -> Vec<Asteroid>
    where
        R: Rng + ?Sized,
    {
    ...
    pub fn interact_blast<R: Clone>(&self, rng: &mut R, blast: &Blast) -> Option<Impact>
    where
        R: Rng + ?Sized,
    {
...
fn fracture_line<R>(rng: &mut R, segment: (Point, Point)) -> Vec<(Point, Point)>
where
    R: Rng + ?Sized,
{
